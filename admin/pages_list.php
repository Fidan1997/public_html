...
<style>
	...
	.linkd{background:rgba(0,0,0,0.05); padding:10px;}
...
<form id="form1" name="form1" method="post">
  <table style="width:90%; max-width:1200px;" align="center" border="0" cellspacing="0" cellpadding="6">
    <tbody>
      ...
      <tr>
        <td colspan="4"><h1>Bütün səhifələr</h1></td>
      </tr>
      <tr>
        <td colspan="4">
        		
                <?
				$sql = mysql_query("select * from `stat` order by `dt` desc") or die(mysql_error());
				
				while($rw = mysql_fetch_array($sql)){
					
					?>
                    <div class="linkd">
                    <a class="link" href="editpage.php?id=<?=$rw['id']?>&token=<?=$_SESSION['token'];?>"><?=$rw['pageheader'];?></a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <i><?=$rw['site'];?></i></div>
                    <?
